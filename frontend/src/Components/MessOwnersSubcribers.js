...
export default function MessOwnersSubcribers() {
  ...
  const fetch_agents = async (e) => {
    await 
    axios
      .post("http://localhost:5000/Mess_owner/fetch_agents/",
      {
        Mess_id:Mess_id
      }
      )
  ...
  const send_request = async (new_agent_id) => {
    await 
    axios
      .post("http://localhost:5000/Mess_owner/send_request/",
      {
        "Mess_id":Mess_id,
      ...
      .then((res) => {
        console.log(res.data[0].sum);
        set_total_tokens(res.data[0].sum);
      }).catch((err)=>{alert("You have already sent request to this agent!");});
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center mb-8">
          <button
            ...
            className={`px-6 py-3 rounded-tl-md rounded-bl-md focus:outline-none ${activeTab === 'messSubscribers' ? 'bg-blue-500 text-white' : 'bg-cyan-100 text-gray-700'}`}
          >
            Mess Subscribers
          </button>
          <button
            onClick={() => handleTabChange('deliveryAgents')}
            className={`px-6 py-3 rounded-tr-md rounded-br-md focus:outline-none ${activeTab === 'deliveryAgents' ? 'bg-blue-500 text-white' : 'bg-cyan-100 text-gray-700'}`}
          >
            Delivery Agents
          </button>
          <button
            ...
            <table className="w-full divide-y divide-cyan-200">
              <thead className="bg-cyan-100 dark:bg-cyan-700">
                <tr className="">
                  ...
                  <th className="px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center">Customer Address</th>
                  <th className="px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center">Contact Number</th>
                  <th className="px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center">Daily_tokens</th>
                </tr>
              </thead>
              <tbody className="bg-cyan-100 divide-y divide-gray-200">
                {mess_users.map((ele) => (<tr>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">{ele.user_address}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">+91 {ele.phone_num}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">{ele.daily_tokens }</td>
                </tr>))}
        ...
        {activeTab === 'deliveryAgents' && (
          <div className="bg-white shadow-lg rounded-lg overflow-hidden mb-8">
            ...
            <table className="w-full divide-y divide-gray-200">
              <thead className="bg-cyan-100 dark:bg-cyan-700">
                <tr>
                  <th className="px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center">Name</th>
                  <th className="px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center">Phone Number</th>
                  <th className="px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center">Email</th>
                  <th className="px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center">Address</th>
                  <th className="px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center">Status</th>
                </tr>
              </thead>
              
              <tbody className="bg-cyan-100 divide-y divide-gray-200">
                {agents.map((ele,index) => (<tr>
                  <td className="px-6 py-4 whitespace-nowrap text-center">{ele.fname } {ele.lname}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">+91 {ele.phone_num}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">{ele.email}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">{ele.user_address}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">{ele.status==='Hired'?<div className="text-orange-800">Hired</div>
                  :(!ele.status)?
  ...
  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
        ...
        {activeTab==="deliveryAgents"&&<Dismiss Mess_id={Mess_id} current_agent={current_agent} update_current_agent={update_current_agent}/>}
      </div>
    </div>
  );
}
